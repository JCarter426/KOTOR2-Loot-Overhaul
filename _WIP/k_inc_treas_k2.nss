...
else if( nItemType == 111 && nItemNum == -1 )
	return "w_blaste_22";
...
/*	LOOT_FindBinaryTerm()
	...
	representation of a sum of powers of 2)
	
	- nBitmask: Number that's a sum of powers of 2
...
switch( nRoll ) {
	...
	case 22: // *Onasi Blaster*
		...
		if( nItemLevel >= 26 ||	LOOT_GetUniqueFound(111, -1) == TRUE )
			nRoll = 26;
		else
	...
	case 30: // *Freedon Nadd's Blaster*
		...
		if( LOOT_GetUniqueFound(111, nRoll) == TRUE )
			nRoll = Random(6) + 24;
		else
...
switch( nRoll ) {
	...
	case 30: // *Freyyr's Warblade*
		// If it was found before, replace with a random high level item
		if( LOOT_GetUniqueFound(131, nRoll) == TRUE  )
			nRoll = Random(10) + 20;
		else
...
switch( nRoll ) {
	...
	case 27: // *Qel-Droma Belt*
		...
		if( LOOT_GetUniqueFound(311, nRoll) == TRUE )
			nRoll = 30;
		else
...
switch( nRoll ) {
	...
	case 27: // *Ossluk's Gloves*
		// If it was found before, replace with a random high level item
		if( LOOT_GetUniqueFound(321, nRoll) == TRUE ) {
			nRoll = Random(8) + 22;
			if( nRoll == 27 )
				nRoll = 30;
			}
...
switch( nRoll ) {
	...
	case 15: // *Ulic Qel-Droma's Mesh Suit*
		// If it was found before, replace with a random high level item
		if( LOOT_GetUniqueFound(411, 15) == TRUE )
			nRoll = 14 - Random(6);
		else
...
switch( nRoll ) {
	...
	case 15: // *Jamoh Hogra's Battle Armor*
		...
		if( LOOT_GetUniqueFound(421, 15) == TRUE )
			nRoll = 9;
		else
...
if( nRoll >= 23 ) {
	...
	if( LOOT_GetUniqueFound(441, nRoll) == TRUE ) {
		...
		if( nGlobal == -1 )
			...
			while( i >= 23 ) {
				if( LOOT_FindBinaryTerm(nGlobal, LOOT_UniqueItemID(441, i)) == FALSE ) {
					nRoll = i;
					i = 0;
					}
				else
			...
			while( i > 0 && j <= 30 ) {
				if( LOOT_FindBinaryTerm(nGlobal, LOOT_UniqueItemID(441, j)) == FALSE ) {
					nRoll = j;
					i = 0;
					}
				else
