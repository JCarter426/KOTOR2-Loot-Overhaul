...
*/
string VP_RandomGenSaber(string sItemLevel){
	int nSaber = Random(100) +1;


	if(nSaber <= 50){
		sItemLevel = "0" + IntToString(Random(4) + 1);
	}
	else if(nSaber > 50 && nSaber <= 80){
		switch(Random(3)){
			case 0:
				sItemLevel = "08";
			break;
...
string VP_RandomGenCrys(string sItemLevel){
	int nSaber = Random(100) +1;
...
string HH_SecSpike(string sTemplate){
	int nSecur = Random(11);

	if(nSecur <= 1){
		sTemplate = "g_i_secspike02";
	}
	else{
            ...
            // item class is known, but sub-class isn't, so we generate it randomly
            {
                ...
                // lightsabers use a different curve
                        ...
                        else
                        {
                            ...
                            {
                                case 141:
                                      /*nItemLevel = (nItemLevel)/3; Commented out to support my randomsaber function.
				      // no number 6
                                      nItemLevel = Random(10) + 1;
				      if(nItemLevel >= 6) nItemLevel += nItemLevel;
                                      sItemLevel = IntToString(nItemLevel);
                                      if(nItemLevel < 10) sItemLevel = "0" + sItemLevel;*/
                                      sItemLevel = VP_RandomGenSaber(sItemLevel);
                                      break;
                                case 142:
				      ...
				      if(nItemLevel >= 6) nItemLevel += nItemLevel;
                                ...
                                case 245:
                                      ...
                                      sItemLevel = IntToString(nItemLevel);
                                      if(nItemLevel < 10) sItemLevel = "0" + sItemLevel;*/
                                      sItemLevel = VP_RandomGenCrys(sItemLevel);
                                      break;
								// Gloves has no 03 fix (1.8.3) -HH
								case 321:
                                    if(nItemLevel<1) nItemLevel = 1;
									if(nItemLevel==3) nItemLevel = 2;
                                    sItemLevel = IntToString(nItemLevel);
                                    if(nItemLevel < 10) sItemLevel = "0" + sItemLevel;
                                    break;
                                case 341:
                            ...
                            case 441:
                                {
                                    nItemLevel = (nItemLevel);
                                    if(nItemLevel < 2) nItemLevel = 2;
                                    sItemLevel = IntToString(nItemLevel);
                                    if(nItemLevel < 10) sItemLevel = "0" + sItemLevel;
                                    break;
                                }
...
string GetBundlePrefix (int nItemLevel, int nItemType)
{
    ...
    switch (nItemType)
    {
        ...
        {
            if(nItemLevel < 11)
                sTemplate = "g_i_cmbtshot001";
                else if(nItemLevel < 21)
                    sTemplate = "g_i_cmbtshot002";
                    else sTemplate = "g_i_cmbtshot003";
            break;
        }
        ...
        case 983:
        {
			sTemplate = HH_SecSpike(sTemplate);
			break;
        }
        case 991:
		{
		...
		case 996:
        {
            sTemplate = "w_rocket_06";
            break;
        }
		case 997:
        {
            sTemplate = "w_rocket_07";
            break;
        }
		case 998:
        {
            sTemplate = "w_rocket_08";
            break;
        }
		case 999:
        {
            sTemplate = "w_rocket_09";
            break;
        }
    }
                ...
                if((nItemType % 10) == 0)
                    {
...

                        {
                            ...
                            case 95: {nItemRandom = 5; break;}
                            case 97: {nItemRandom = 9; break;}
                            case 98: {nItemRandom = 3; break;}
                            // increase access to rockets as level increases (-Removed since not working, -HH)
                            case 99: {nItemRandom = 9; break;}
...
{
	int nRareLootFix = nItemLevel + 5;
	if(nRareLootFix >30) nRareLootFix = 30 - Random(3);
	nItemLevel = nRareLootFix;
    string sTemplate = GetTreasureSpecific(nItemLevel, nItemType);
